...
ROOT="${1:-/root/lr_screen_50ep}"
SOURCE_ROOT="${SOURCE_ROOT:-/root/lr_screen_30ep}"
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/bin/python}"
BASE_CONFIG="$PROJECT_DIR/configs/cifar10_lr_screen_50ep_resume_base.yaml"
...
for SPEC in "${SPECS[@]}"; do
  ...
  RESUME="$SOURCE_ROOT/runs/$LABEL/ckpt/final.pt"
  mkdir -p "$OUT/ckpt" "$OUT/samples"
  cp "$BASE_CONFIG" "$CONFIG"
  sed -i "s#^output_dir:.*#output_dir: $OUT#; s#^  lr:.*#  lr: $LR#" "$CONFIG"

  if [ ! -f "$RESUME" ]; then
    echo "$LABEL resume checkpoint missing: $RESUME" | tee -a "$LOG"
    echo "===== END $LABEL =====" | tee -a "$LOG"
    continue
  fi

  echo "===== START $LABEL =====" | tee -a "$LOG"
  echo "[LR_RESUME] label=$LABEL lr=$LR resume=$RESUME output=$OUT" | tee -a "$LOG"
  set +e
  ...
  echo "[LR_RESUME] quick FID start label=$LABEL" | tee -a "$LOG"
  set +e
...
echo "[LR_RESUME] all candidates finished" | tee -a "$LOG"
