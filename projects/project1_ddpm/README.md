...
为检验 30 epoch 粗筛中较高学习率是否会在更长训练阶段改善，已将 LR `2.5e-4` 和
`3.0e-4` 从各自的 30 epoch checkpoint 继续训练到 50 epoch，共新增 `7,820` 个有效更新，
总计 `19,550` 步。训练配置、轻量日志和 FID 文本见
[`results/lr_screen_50ep/`](results/lr_screen_50ep/)。
