...
## 附录 B：LR=5e-5 完整 200 epoch 复核

为验证粗筛中 LR=5e-5 的候选，保持完整 U-Net、cosine beta、uniform epsilon MSE、seed44、
200 epoch、78,000 steps、5,000 steps warmup、58,500 steps 后期衰减和 EMA bank 不变，
将峰值学习率改为 `5e-5`，并按相同 10% 比例将最小学习率设为 `5e-6`。正式协议仍为
train split 前 5,000 张真实图、5,000 张生成图、seed44、逐步裁剪预测 x0。
...
该实验说明：30 epoch 粗筛的“LR=5e-5 最优”只适合作为早期排除高学习率的信号，不能
预测完整 200 epoch 的最终 FID。相同正式协议下，LR=5e-5 反而比 R5 明显更差，说明在
当前 200 epoch 预算内，R5 的 `2e-4` 峰值学习率更有利于完成有效拟合。EMA 0.9999 仍
显著优于 raw，但参数平均不能弥补峰值学习率过低造成的有效学习不足。本次约在 step
76,471 发生一次 AMP optimizer step skip，训练随后正常完成，没有 OOM、NaN 或 checkpoint
损坏证据，因此不能将其作为 FID 差距的主要原因。

需要注意：本次将 `min_lr` 按峰值比例设为 `5e-6`，而 R5 的绝对 `min_lr` 为 `2e-5`；
所以该结果严格回答的是“低峰值 + 同比例后期衰减”是否更好，不能单独分离最小学习率
绝对值的影响。完整轻量结果见 `results/fid15_lr5e5_full/`，大型 checkpoint 位于
`challenge-v1 Release`。

## 附录 C：30→50 epoch 学习率续训粗筛

为进一步判断 30 epoch 粗筛的学习率排序是否稳定，将其中的 LR `2.5e-4` 和 `3.0e-4`
从各自的 30 epoch `final.pt` 继续训练到 50 epoch。两组保持完整 U-Net、cosine beta、
epsilon prediction、uniform loss、seed 44、batch 128、1,000 steps warmup、warmup 后
固定学习率以及 EMA bank 不变。每组从 `11,730` 步继续到 `19,550` 步，实际新增
`7,820` 个有效更新，未从头训练。

本轮仍使用快速筛选协议：1,000 张生成图对 1,000 张 train split 真实图、seed 44、EMA
0.9999、逐步裁剪预测 `x0`。结果如下：

| 学习率 | 30 epoch 快速 FID | 50 epoch 快速 FID | 变化 |
|---:|---:|---:|---:|
| `2.5e-4` | 330.7679 | **193.6462** | -137.1217 |
| `3.0e-4` | 323.4893 | 202.6441 | -120.8452 |

两组均正常完成，最后记录的 loss 分别为 `0.04731` 和 `0.04724`，每组训练耗时约
16.1 分钟。日志中可见少量 AMP optimizer step skip，但 scaler 随后恢复，未见 OOM、
NaN 或 checkpoint 损坏。50 epoch 下 `2.5e-4` 优于 `3.0e-4`，因此若继续正式复核应
优先选择 `2.5e-4`。

这组数值仍不是正式 FID，不能直接与 R5 的 5,000/5,000、200 epoch 正式结果 `15.4385`
比较。它说明更高学习率在早期延长训练中仍能持续改善，但不能证明其在完整 200 epoch、
后期学习率衰减和固定正式协议下会低于 15。轻量配置、日志和 FID 文本见
`results/lr_screen_50ep/`；大型最终 checkpoint 不进入普通 Git。

## 附录 D：LR=2.5e-4 完整 200 epoch 复核

为验证 50 epoch 粗筛中表现最好的 LR `2.5e-4`，从其 `19,550` 步 checkpoint 继续到总计
200 epoch、`78,000` 步，新增 `58,450` 个有效更新。除学习率外保持 R5 的完整 U-Net、
cosine beta、epsilon prediction、uniform loss、seed44、batch128、EMA bank 和裁剪 `x0`
协议；峰值学习率为 `2.5e-4`，step `58,500` 后按 10% 比例衰减到 `2.5e-5`。

正式 FID 使用 5,000 张生成图与 train split 前 5,000 张真实图，seed44：

| 权重 | LR=2.5e-4 复核 | R5 LR=2e-4 | 差值 |
|---|---:|---:|---:|
...
LR `2.5e-4` 的 EMA 0.999/0.9995 在本次复核中低于 R5 对应 EMA，但最佳 EMA 0.9999
仍高于 R5。由于最终选择通常使用 EMA 0.9999，本实验不能证明提高峰值学习率有益；它
反而说明 50 epoch 快速 FID 的排序不能直接外推到完整 200 epoch。训练耗时约 `122.5`
分钟，期间只有少量 AMP step skip，没有 OOM、NaN 或 checkpoint 损坏。完整轻量结果见
`results/lr25_200ep/`，大型 checkpoint 通过 `challenge-v1 Release` 提供。
