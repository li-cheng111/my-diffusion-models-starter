...
为验证 30 epoch 粗筛得到的 LR=5e-5 候选，执行了完整 200 epoch、78,000 steps 训练，
并使用与 R5 相同的正式 FID 协议。峰值学习率为 `5e-5`，warmup 5,000 steps，step
58,500 后衰减至 `5e-6`；其余模型、seed、beta、loss、EMA 和采样设置保持一致。
...
结论是：低学习率在 30 epoch 粗筛中表现较好，但在完整 200 epoch 下并未优于 R5；
